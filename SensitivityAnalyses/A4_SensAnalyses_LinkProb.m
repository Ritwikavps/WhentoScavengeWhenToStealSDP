...
%Feb 2023: This is a modification of the baseline script used to run the sensitivity analyses. Here, we test the effect of scaling the alpha, beta,
% and gamma parameters in the Rohr et al eqn to compute the probability of a food web link between predator and prey. Note the use of the
% RohrScaleFactor variable to do this. Everything else remains the same: a linear fitness function, a 12 hr:12 hr split of activity and rest, 
...
RohrScaleFactorVec = 0.5:0.2:1.5; %factor to scale the Rohr linking probabilty parameters by
RohrScalePerc = RohrScaleFactorVec*100; %percentage value of the scaling factor to name files

for i = 1:numel(RohrScaleFactorVec) %go through linking prob. scale factors

    %generatw allometric quantities
    AllometricStruct = GenerateAllomtericQuantities_SensAnalysesLinkProb(Mp,Mr,Mc,RohrScaleFactorVec(i));
    ...
    for trial = 1:5 
        ...
        %loops to make name
        fitsavename = sprintf('FitDecExpfitness_SensLinkProbMarch2023_ScalePerc%i_%i.mat',RohrScalePerc(i),trial);
        countssavename = sprintf('StrategyCountsExpfitness_SensLinkProbMarch2023_ScalePerc%i_%i.mat',RohrScalePerc(i),trial);
        probsavename = sprintf('SdpEncProbExpfitness_SensLinkProbMarch2023_ScalePerc%i_%i.mat',RohrScalePerc(i),trial);
