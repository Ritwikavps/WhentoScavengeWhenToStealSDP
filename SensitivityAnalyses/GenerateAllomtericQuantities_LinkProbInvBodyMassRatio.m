function AllometricStruct = GenerateAllomtericQuantities_LinkProbInvBodyMassRatio(Mp,Mr,Mc)

%generates structure containing metabolic rates, fat mass, muscle mass,
%etc. using allometric relationships, modified for using the inverse mass
%ratio in the Rohr equation to compute the probability of a food web link
...
for pp = 1:lp
    for rr = 1:lr
        ...
        exponent_con = a + b.*log10(Mr(rr)./Mp(pp)) + g.*(log10(Mr(rr)./Mp(pp))).^2; %there is a link probability for each predator mass
