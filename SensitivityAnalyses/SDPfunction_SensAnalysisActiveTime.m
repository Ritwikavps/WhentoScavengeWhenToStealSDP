function [FitnessMatrix,DecisionMatrix,HuntCount,ScavCount,KlepCount] = SDPfunction_SensAnalysisActiveTime(Mp,Mr,Mc,tvec,AllometricStruct,...
                                    pp,rr,cc,KlepEncNumVec,KlepEncProbVec,HuntEncNumVec,HuntEncProbVec,ScavEncNumVec,ScavEncProbVec,FitnessFuncType,ActiveTimeScaleFactor)

%function that does SDP computation, modified so that we account for a
%split of the 24-hr day that is not 12 hour rest and 12 hour active time. 
%Note the use of the ActiveTimeScaleFactor variable in the computation of
%the updated energetic content for hunting (x_h), scavenging (x_scav), and
%stealing (x_klep)
...
for i = 1:numel(HuntEncNumVec)
    ...
    x_h(:,i) = x - AllometricStruct.Bmr12hr_p(pp) - ...
        (12*60*60*ActiveTimeScaleFactor - 0.2*AllometricStruct.ThandleSec_pr(pp,rr)*HuntEncNumVec(i))*AllometricStruct.FmrJperSec_p(pp) -...
...
for i = 1:numel(ScavEncNumVec)
    x_scav(:,i) = x - AllometricStruct.Bmr12hr_p(pp) - 12*60*60*ActiveTimeScaleFactor*AllometricStruct.FmrJperSec_p(pp) + ...
...
for i = 1:length(KlepEncNumVec)
    x_klep(:,i) = x - AllometricStruct.Bmr12hr_p(pp) - 12*60*60*ActiveTimeScaleFactor*AllometricStruct.FmrJperSec_p(pp) +...
...
for t_in = length(tvec)-1:-1:1 %start at terminal time - 1 because no decisions at terminal time
    ...
    mu_h = (24*60*60 + 0.2*AllometricStruct.ThandleSec_pr(pp,rr)*HuntEncNumVec*((1 + Mr(rr)/Mp(pp)) - 1))*AllometricStruct.MuPerSec_p(pp); 
    FitHuntWeighted = fx_h*transpose((HuntEncProbVec).*(1-mu_h));
    
    %kleptoparasitsim mortality
    mu_k = (24*60*60 + 0.1*AllometricStruct.ThandleSec_pr(pp,rr)*KlepEncNumVec*((1 + 2*Mc(cc)/Mp(pp)) - 1))*AllometricStruct.MuPerSec_p(pp);
