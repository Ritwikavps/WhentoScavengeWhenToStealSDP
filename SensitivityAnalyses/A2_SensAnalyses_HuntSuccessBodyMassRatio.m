%Ritwika VPS
%UC Merced
%SDP sensitivity analyses -- baseline

%Feb 2023: This is a version of the script used to run the SDP to get results for sensitivity analyses, where instead of the Rohr link prob to determine the
% prob. of hunting success, we use a simply body mass ratio. That is, if predator is learger than prey, hunting success is 1; if not, it is given by Mp/Mr. So, this utilises an
%exponential fitness function, a 12 hr:12 hr split of activity and rest, and
...
for trial = 1:5 
        ...
        for pp = 1:lp %loops over consumer mass
            for rr = 1:lr %loop over prey mass
                ...
                [KlepEncNum{pp,rr,cc},KlepEncProb{pp,rr,cc}] = ComputeEncounterProb_Klep(trialnum,Mp(pp),Mr(rr),Mc(cc),ActiveTime,ActiveTime,...
                                                                                         min(Mc(cc)/Mr(rr),1)); 
                [HuntEncNum{pp,rr,cc},HuntEncProb{pp,rr,cc}] = ComputeEncounterProb_Hunting(Mp(pp),Mr(rr),trialnum,ActiveTime,min(Mp(pp)/Mr(rr),1));
                [ScavEncNum{pp,rr,cc},ScavEncProb{pp,rr,cc}] = ComputeEncounterProb_Scav(trialnum,Mp(pp),Mr(rr),Mc(cc),ActiveTime,...
                                                                                        ActiveTime,min(Mc(cc)/Mr(rr),1));
    ...
    %loops to make name
    fitsavename = sprintf('FitDecExpfitness_SensHuntSuccessBodyMassRatioJun2023_%i.mat',trial);
    countssavename = sprintf('StrategyCountsExpfitness_SensHuntSuccessBodyMassRatioJun2023_%i.mat',trial);
    probsavename = sprintf('SdpEncProbExpfitness_SensHuntSuccessBodyMassRatioJun2023_%i.mat',trial);
