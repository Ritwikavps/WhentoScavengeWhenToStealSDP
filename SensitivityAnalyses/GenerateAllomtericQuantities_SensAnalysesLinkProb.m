function AllometricStruct = GenerateAllomtericQuantities_SensAnalysesLinkProb(Mp,Mr,Mc,RohrScaleFactor)

%generates structure containing metabolic rates, fat mass, muscle mass,
%etc. using allometric relationships, modified to scale the alpha, beta, and gamma
% parameters in the Rohr equation to compute the probability of a food web link
%between predator and prey (see Supp Info of the paper for more context).
%Note the use of the RohrScaleFactor variable to do this.
...
a = RohrScaleFactor*2.51; %to determine linking probability between carnivore and prey; %units don't matter because ratio of masses is used
b = RohrScaleFactor*0.79;
g = RohrScaleFactor*(-0.37);
